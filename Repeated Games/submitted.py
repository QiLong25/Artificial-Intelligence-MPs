...
def utility_partials(R, x):
    ...
    du_dpol_0 = (R[0, :, :] @ sig2(x[1])).T             # du / dpolicy
    du_dpol_1 = sig2(x[0]) @ R[1, :, :]

    dpol_dx_0 = dsig2(sig2(x[0]))
    dpol_dx_1 = dsig2(sig2(x[1]))

    partial = np.zeros(2)
    partial[0] = du_dpol_0.reshape((1, -1)) @ dpol_dx_0.reshape((-1, 1))
    partial[1] = du_dpol_1.reshape((1, -1)) @ dpol_dx_1.reshape((-1, 1))
...
def episodic_game_gradient_ascent(init, rewards, nsteps, learningrate):
    ...
    for step in range(nsteps):
        logits[step] = init
        utilities[step][0] = sig2(init[0]) @ rewards[0, :, :] @ sig2(init[1])
        utilities[step][1] = sig2(init[0]) @ rewards[1, :, :] @ sig2(init[1])

        ## calculate gradient
        grad = utility_partials(rewards, init)              # [du_dx_0, du_dx_1]

        ## udpate x
        init += learningrate * grad
...
def utility_hessian(R, x):
    ...
    dpol_dx_0 = dsig2(sig2(x[0]))           # first derivative
    dpol_dx_1 = dsig2(sig2(x[1]))

    dpol_dx_0_2 = Hsig2(sig2(x[0]))         # second derivative
    dpol_dx_1_2 = Hsig2(sig2(x[1]))

    hessian = np.zeros((2, 2))
    hessian[0, 1] = dpol_dx_0.reshape((1, -1)) @ R[0, :, :] @ dpol_dx_1.reshape((-1, 1))
    hessian[1, 0] = dpol_dx_1.reshape((1, -1)) @ R[1, :, :] @ dpol_dx_0.reshape((-1, 1))
    hessian[0, 0] = du_dpol_0.reshape((1, -1)) @ dpol_dx_0_2.reshape((-1, 1))
    hessian[1, 1] = du_dpol_1.reshape((1, -1)) @ dpol_dx_1_2.reshape((-1, 1))
...
def episodic_game_corrected_ascent(init, rewards, nsteps, learningrate):
    ...
    for step in range(nsteps):
        ...
        ## calculate gradient
        partials = utility_partials(rewards, init)  # [du_dx_0, du_dx_1]

        ## calculate C
        hessian = utility_hessian(rewards, init)
        C = symplectic_correction(partials, hessian)

        ## udpate x
        init += learningrate * (np.identity(C.shape[0], dtype=float) + C) @ partials
...
sequential_strategy = np.array([[0.0, 0.8],[0.2, 1.0]])
