...
def get_returns(rollout_buffer: utils.RolloutBuffer, discount_factor=0.95):
    ...
    """
    # YOUR CODE HERE
    result = torch.zeros(len(rollout_buffer.rewards))
    idx = len(rollout_buffer.rewards) - 1           # start from the end
    cumulate_reward = 0
    while idx >= 0:
        if rollout_buffer.terminateds[idx]:             # reach end of traj
            cumulate_reward = 0
        cumulate_reward = rollout_buffer.rewards[idx] + discount_factor * cumulate_reward
        result[idx] = cumulate_reward
        idx -= 1
...
def get_advantages(value_net: nn.Module,
    ...
    with torch.no_grad():
        V = value_net(observations).reshape(-1, 1)
        adv = returns - V
        adv_std = (adv - torch.mean(adv, dim=0, keepdim=True)) / (torch.std(adv, dim=0, keepdim=True) + EPSILON)
...
def get_value_net_loss(value_net: nn.Module,
    ...
    # YOUR CODE HERE
    mse_loss = nn.MSELoss()
    pred = value_net(observation).reshape(-1, 1)
    loss = mse_loss(pred, returns)
    return loss
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    """
    # YOUR CODE HERE
    total_J = torch.sum(policy(observation).gather(1, action).reshape(-1) * return_or_advantage.reshape(-1))
    result = - 1 / observation.shape[0] * total_J
    return result
...
def collect_rollouts(env: utils.EnvInterface,
    ...
    for i in range(num_rollouts):
        ...
        while not terminated:
            ...
            obs_next, terminated, reward = env.step(action)
    ...
    while r < rollouts:
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                if get_advantages:
                    return_or_advantage = advantages[batch_start:batch_stop]
                else:
                    return_or_advantage = returns[batch_start:batch_stop]
...
def get_PPO_policy_gradient_loss (policy: nn.Module,
    ...
    # YOUR CODE HERE
    new_logits = policy(observation).gather(1, action).reshape(-1)

    ratio = (new_logits - old_logits.gather(1, action).reshape(-1)).reshape(-1, 1)
    ratio = torch.exp(ratio)

    adv = ratio * return_or_advantage
    clip_adv = torch.clamp(ratio, min=1-ppo_clip, max=1+ppo_clip) * return_or_advantage

    result = torch.mean(-torch.min(adv, clip_adv))
